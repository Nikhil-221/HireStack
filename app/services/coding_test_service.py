...
class CodingTestService:
    ...
    def create_test(self, payload: CodingTestCreate) -> dict:
        ...
        coding_test = self.repository.create(
            {"job_id": job.id, "title": payload.title, "duration_minutes": payload.duration_minutes},
    ...
    def _validate_question_selection(self, job_id: int, question_ids: Sequence[int]):
        ...
        cross_job = [question.id for question in questions if question.job_id != job_id]
        if cross_job:
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail="All questions must belong to the selected job",
            )
        return job
    ...
    def _serialize(self, coding_test: CodingTest, include_questions: bool = True) -> dict:
        ...
        return {
            ...
            "job_title": self.repository.get_job(coding_test.job_id).title,
