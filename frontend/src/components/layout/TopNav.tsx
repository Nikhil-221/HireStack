    ...
    <header className={`sticky top-0 z-50 border-b backdrop-blur ${isDark ? 'border-white/10 bg-slate-950/85 text-white' : 'border-slate-200/80 bg-white/85 text-slate-900'}`}>
      <div className="mx-auto flex min-h-16 max-w-[1440px] items-center justify-between gap-3 px-4 sm:px-6 lg:px-8">
        ...
        <nav className="flex min-w-0 items-center justify-end gap-0.5 overflow-x-auto py-2">
          ...
          {recruiter && <><NavLink to="/dashboard" className={({ isActive }) => linkClass(isDark, isActive)}>Dashboard</NavLink><NavLink to="/jobs" className={({ isActive }) => linkClass(isDark, isActive)}>Jobs</NavLink><NavLink to="/question-bank" className={({ isActive }) => linkClass(isDark, isActive)}>Question Bank</NavLink><NavLink to="/coding-tests" className={({ isActive }) => linkClass(isDark, isActive)}>Coding Tests</NavLink></>}
