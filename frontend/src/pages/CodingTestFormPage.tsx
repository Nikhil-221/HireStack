...
export function CodingTestFormPage() {
  ...
  useEffect(() => {
    ...
    fetchCodingTest(Number(id))
      .then((test) => {
        setJobId(test.job_id ? String(test.job_id) : ALL_JOBS_VALUE)
        setTitle(test.title)
  ...
  useEffect(() => {
    ...
    fetchCodingQuestions(jobId === ALL_JOBS_VALUE ? {} : { job_id: Number(jobId) })
      .then(setQuestions)
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    setIsSaving(true)
    setError(null)
    const payload: CodingTestPayload = { job_id: jobId === ALL_JOBS_VALUE ? null : Number(jobId), title: title.trim(), duration_minutes: Number(duration), question_ids: selectedQuestionIds }
    try {
